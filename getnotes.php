	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		if(isset($_SESSION['email'])){
			...
			$jd = cal_to_jd(CAL_GREGORIAN,date("m"),date("d"),date("Y"));
		 	...
		 	$query ="select n.noteid,n.notedesc,nf.lat,nf.longg,uf.radius,nf.rad
					from user u, notes n, userpref uf, notefilter nf, scheduler s, days d, tag t1, tag t2, utag ut, ntag nt
					where u.userid = '$a' and uf.state = '$b' and u.userid = uf.userid and n.noteid = nf.noteid and s.sid = d.sid and s.nid = nf.nid and t1.tagid = ut.tagid and ut.userid = uf.userid and uf.state = ut.state and nt.tagid = t2.tagid and nf.nid = nt.nid and t1.tagid = t2.tagid and (s.fromdate <= CURRENT_DATE  and CURRENT_DATE <= s.todate) and (d.daay = 'everyday' or d.daay = '$curday') and (d.fromtime <= CURRENT_TIME and CURRENT_TIME <= d.totime) and (uf.stime <= CURRENT_TIME and CURRENT_DATE <= uf.etime)";
		 	$rs = mysqli_query($conn,$query);
		 	...
		 	if($rs){
				...
				while( $count < (int)$val ){
					if($row = mysqli_fetch_assoc($rs)){
						$be = getd($lat,$long,$row["lat"],$row["longg"]);
